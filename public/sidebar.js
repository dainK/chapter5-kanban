...
export async function showSidebarState() {
...
async function showSideStatelogin(username) {
  ...
  document.getElementById('board-list-text').style.display = 'block';
  loadBoardList();
}
...
function showSideStatelogout() {
  ...
  document.getElementById('board-list-text').style.display = 'none';
}
...
function createSidebar() {
  ...
  logoutButton.addEventListener('click', () => {
    const accessToken = localStorage.getItem('access_token');
    axios
      .post('/user/logout', {}, { headers: { Authorization: `Bearer ${accessToken}` } })
      .then((response) => {
        localStorage.removeItem('access_token');
        location.reload();
      })
